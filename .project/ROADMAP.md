...
- [x] **Add frontend E2E tests** — Set up Playwright with 3 E2E test suites (admin-api.spec.ts, crud-flows.spec.ts, dashboard-ui.spec.ts) covering: admin API authentication (valid/invalid/rejected keys), login page rendering, services/upstreams/routes/users CRUD operations, dashboard page loading, health endpoint validation. Config: `web/playwright.config.ts` with Chromium project, webServer auto-start, trace on retry, screenshot on failure. Scripts: `npm run e2e`, `npm run e2e:ui`, `npm run e2e:headed`, `make e2e-web`. Test config: `test/e2e-config.yaml` with in-memory SQLite.
